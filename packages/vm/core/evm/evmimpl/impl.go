...
func registerERC20NativeToken(ctx isc.Sandbox) dict.Dict {
	foundrySN := lo.Must(codec.DecodeUint32(ctx.Params().Get(evm.FieldFoundrySN)))
	name := lo.Must(codec.DecodeString(ctx.Params().Get(evm.FieldTokenName)))
	tickerSymbol := lo.Must(codec.DecodeString(ctx.Params().Get(evm.FieldTokenTickerSymbol)))
	decimals := lo.Must(codec.DecodeUint8(ctx.Params().Get(evm.FieldTokenDecimals)))
...
func registerERC20NativeTokenOnRemoteChain(ctx isc.Sandbox) dict.Dict {
	...
	tickerSymbol := lo.Must(codec.DecodeString(ctx.Params().Get(evm.FieldTokenTickerSymbol)))
	decimals := lo.Must(codec.DecodeUint8(ctx.Params().Get(evm.FieldTokenDecimals)))
	target := lo.Must(codec.DecodeAddress(ctx.Params().Get(evm.FieldTargetAddress)))
	...
	tokenScheme := func() iotago.TokenScheme {
		...
		o := lo.Must(codec.DecodeOutput(res[accounts.ParamFoundryOutputBin], ctx.L1API()))
...
func registerERC20ExternalNativeToken(ctx isc.Sandbox) dict.Dict {
	...
	foundrySN := lo.Must(codec.DecodeUint32(ctx.Params().Get(evm.FieldFoundrySN)))
	tokenScheme := lo.Must(codec.DecodeTokenScheme(ctx.Params().Get(evm.FieldFoundryTokenScheme)))
	simpleTS, ok := tokenScheme.(*iotago.SimpleTokenScheme)
	if !ok {
		panic(errUnsupportedTokenScheme)
	}
	accountAddress := lo.Must(codec.DecodeAddress(ctx.Params().Get(evm.FieldTargetAddress)))
...
func viewERC20ExternalNativeTokenAddress(ctx isc.SandboxView) dict.Dict {
	nativeTokenID := lo.Must(codec.DecodeNativeTokenID(ctx.Params().Get(evm.FieldNativeTokenID)))
...
func registerERC721NFTCollection(ctx isc.Sandbox) dict.Dict {
	collectionID := lo.Must(codec.DecodeNFTID(ctx.Params().Get(evm.FieldNFTCollectionID)))
...
func getEVMGasRatio(ctx isc.SandboxBase) util.Ratio32 {
	...
	return lo.Must(codec.DecodeRatio32(gasRatioViewRes.Get(governance.ParamEVMGasRatio), gas.DefaultEVMGasRatio))
}
