...
func TestUploadBlob(t *testing.T) {
	...
	t.Run("several", func(t *testing.T) {
		...
		ret, err := ch.CallView(ch.LatestBlockIndex(), blob.Contract.Name, blob.ViewListBlobs.Name)
		require.NoError(t, err)
		...
		for _, h := range hashes {
			...
			ret, err := ch.CallView(ch.LatestBlockIndex(), blob.Contract.Name, blob.ViewGetBlobField.Name,
				blob.ParamHash, h,
