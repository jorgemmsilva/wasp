...
func TestCruelWorld(t *testing.T) { //nolint:gocyclo
	...
	for i := range sms {
		...
		origin.InitChain(0, stores[i], nil, 0, testutil.TokenInfo, testutil.L1API)
	...
	consensusStateProposalResult := makeNRequests(consensusStateProposalCount, consensusStateProposalDelay, func(_ int) bool {
		...
		responseCh := sms[nodeIndex].ConsensusStateProposal(context.Background(), bf.GetChainOutputs(blocks[blockIndex].L1Commitment()))
		<-responseCh
		return true
	})
	...
	consensusDecidedStateResult := makeNRequests(consensusDecidedStateCount, consensusDecidedStateDelay, func(_ int) bool {
		...
		responseCh := sms[nodeIndex].ConsensusDecidedState(context.Background(), bf.GetChainOutputs(blocks[blockIndex].L1Commitment()))
		state := <-responseCh
	...
	mempoolStateRequestResult := makeNRequests(mempoolStateRequestCount, mempoolStateRequestDelay, func(_ int) bool {
		...
		oldStateOutput := bf.GetChainOutputs(blocks[oldBlockIndex].L1Commitment())
		newStateOutput := bf.GetChainOutputs(blocks[newBlockIndex].L1Commitment())
