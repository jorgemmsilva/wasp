...
func TestPeeringNetReliable(t *testing.T) {
	...
	behavior := NewPeeringNetReliable(testlogger.WithLevel(testlogger.NewLogger(t), log.LevelError))
...
func TestPeeringNetUnreliable(t *testing.T) {
	...
	dstPeerIdentity := cryptolib.NewKeyPair()
	someNode := peeringNode{peeringURL: "src", identity: srcPeerIdentity}
	behavior := NewPeeringNetUnreliable(50, 50, 50*time.Millisecond, 100*time.Millisecond, testlogger.WithLevel(testlogger.NewLogger(t), log.LevelError))
	behavior.AddLink(inCh, outCh, dstPeerIdentity.GetPublicKey())
	for i := 0; i < 1000; i++ {
		inCh <- &peeringMsg{from: someNode.identity.GetPublicKey()}
	}
...
func TestPeeringNetGoodQuality(t *testing.T) {
	...
	dstPeerIdentity := cryptolib.NewKeyPair()
	someNode := peeringNode{peeringURL: "src", identity: srcPeerIdentity}
	behavior := NewPeeringNetUnreliable(100, 0, 0*time.Microsecond, 0*time.Millisecond, testlogger.WithLevel(testlogger.NewLogger(t), log.LevelError)) // NOTE: No drops, duplicates, delays.
	behavior.AddLink(inCh, outCh, dstPeerIdentity.GetPublicKey())
	for i := 0; i < 1000; i++ {
		inCh <- &peeringMsg{from: someNode.identity.GetPublicKey()}
	}
