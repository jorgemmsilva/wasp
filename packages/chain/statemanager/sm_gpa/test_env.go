...
func newTestEnv(
	...
	createSnapMFun func(origStore, nodeStore state.Store, tp sm_gpa_utils.TimeProvider, log log.Logger) sm_snapshots.SnapshotManager,
...
func (teT *testEnv) addNodes(
	...
	createSnapMVariedFun := func(nodeID gpa.NodeID, origStore, nodeStore state.Store, tp sm_gpa_utils.TimeProvider, log log.Logger) sm_snapshots.SnapshotManager {
...
func (teT *testEnv) addVariedNodes(
	...
	for _, nodeID := range nodeIDs {
		...
		origin.InitChain(0, store, teT.bf.GetChainInitParameters(), 0, testutil.TokenInfo, testutil.L1API)
	}
	teT.nodeIDs = nodeIDs
	teT.sms = sms
	teT.snapms = snapms
...
func (teT *testEnv) sendConsensusStateProposal(commitment *state.L1Commitment, nodeID gpa.NodeID) <-chan interface{} {
	input, responseCh := sm_inputs.NewConsensusStateProposal(context.Background(), teT.bf.GetChainOutputs(commitment))
	teT.tc.WithInputs(map[gpa.NodeID]gpa.Input{nodeID: input}).RunAll()
	return responseCh
}
...
func (teT *testEnv) sendConsensusDecidedState(commitment *state.L1Commitment, nodeID gpa.NodeID) <-chan state.State {
	input, responseCh := sm_inputs.NewConsensusDecidedState(context.Background(), teT.bf.GetChainOutputs(commitment))
	teT.tc.WithInputs(map[gpa.NodeID]gpa.Input{nodeID: input}).RunAll()
	return responseCh
}
...
func (teT *testEnv) sendChainFetchStateDiff(oldCommitment, newCommitment *state.L1Commitment, nodeID gpa.NodeID) <-chan *sm_inputs.ChainFetchStateDiffResults {
	input, responseCh := sm_inputs.NewChainFetchStateDiff(context.Background(), teT.bf.GetChainOutputs(oldCommitment), teT.bf.GetChainOutputs(newCommitment))
	teT.tc.WithInputs(map[gpa.NodeID]gpa.Input{nodeID: input}).RunAll()
	return responseCh
}
