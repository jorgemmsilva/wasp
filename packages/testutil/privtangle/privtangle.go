...
func (pt *PrivTangle) nodeClient(i int) *nodeclient.Client {
	client, err := nodeclient.New(fmt.Sprintf("http://localhost:%d", pt.NodePortRestAPI(i)))
	if err != nil {
		panic(fmt.Errorf("failed to create nodeclient %v", err))
	}
...
func (pt *PrivTangle) waitAllReturnTips(timeout time.Duration) {
	...
	for {
		...
		default:
			allOK := true
			for i := range pt.NodeCommands {
				_, err := pt.nodeClient(i).Health(pt.ctx)
				if err != nil {
					allOK = false
				}
			}
