...
func ChainIDFromParams(c echo.Context, cs interfaces.ChainService, l1API iotago.API) (isc.ChainID, error) {
	chainID, err := params.DecodeChainID(c, l1API)
	if err != nil {
		return isc.ChainID{}, err
	}

	if !cs.HasChain(chainID) {
		return isc.ChainID{}, apierrors.ChainNotFoundError(chainID.Bech32(l1API.ProtocolParameters().Bech32HRP()))
	}
...
func ChainFromParams(c echo.Context, cs interfaces.ChainService, l1API iotago.API) (chaintypes.Chain, isc.ChainID, error) {
	chainID, err := params.DecodeChainID(c, l1API)
	if err != nil {
