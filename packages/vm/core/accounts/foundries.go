...
func (s *StateReader) accountFoundriesMap(agentID isc.AgentID) *collections.ImmutableMap {
...
func (s *StateReader) allFoundriesMap() *collections.ImmutableMap {
...
func (s *StateReader) GetFoundryOutput(sn uint32, chainAccountID iotago.AccountID) (*iotago.FoundryOutput, iotago.OutputID) {
	data := s.allFoundriesMap().GetAt(codec.Uint32.Encode(sn))
	if data == nil {
...
func (s *StateReader) hasFoundry(agentID isc.AgentID, sn uint32) bool {
	return s.accountFoundriesMap(agentID).HasAt(codec.Uint32.Encode(sn))
}
