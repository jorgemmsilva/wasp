...
func TestM001Migration(t *testing.T) {
	...
	expectedOriginatorBalance := iotago.BaseToken(17994760)

	res, err := ch.CallView(accounts.ViewBalanceBaseToken.Message(&ch.OriginatorAgentID))
	require.NoError(t, err)
	require.EqualValues(t, expectedOriginatorBalance, lo.Must(codec.Uint64.Decode(res.Get(accounts.ParamBalance))))
