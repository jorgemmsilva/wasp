...
func checkCounter(e *solo.Chain, expected int64) {
	...
	c, err := ViewGetCounter.Output.Decode(ret)
	require.NoError(e.Env.T, err)
...
func TestIncWith1Post(t *testing.T) {
	...
	env.AdvanceTime(600 * time.Second)
...
func TestSpawn(t *testing.T) {
	...
	require.NoError(t, err)
	recs, err := root.ViewGetContractRecords.Output.Decode(res)
	require.NoError(t, err)
	require.True(t, len(recs) == len(corecontracts.All)+2)
}
