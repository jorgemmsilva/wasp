...
func startMaintenance(ctx isc.Sandbox) dict.Dict {
	...
	governance.NewStateWriterFromSandbox(ctx).SetMaintenanceStatus(true)
	return nil
}

func stopMaintenance(ctx isc.Sandbox) dict.Dict {
	ctx.RequireCallerIsChainOwner()
	governance.NewStateWriterFromSandbox(ctx).SetMaintenanceStatus(false)
	return nil
}

func getMaintenanceStatus(ctx isc.SandboxView) bool {
	return governance.NewStateReaderFromSandbox(ctx).MaintenanceStatus()
}
