...
func TestWaitReq(t *testing.T) {
	...
	req0 := isc.NewOffLedgerRequest(isc.RandomChainID(), isc.NewMessageFromNames("foo", "bar"), 0, gas.LimitsDefault.MaxGasPerRequest).Sign(kp)
	req1 := isc.NewOffLedgerRequest(isc.RandomChainID(), isc.NewMessageFromNames("foo", "bar"), 1, gas.LimitsDefault.MaxGasPerRequest).Sign(kp)
	req2 := isc.NewOffLedgerRequest(isc.RandomChainID(), isc.NewMessageFromNames("foo", "bar"), 2, gas.LimitsDefault.MaxGasPerRequest).Sign(kp)
