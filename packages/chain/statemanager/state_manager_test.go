...
func TestCruelWorld(t *testing.T) { //nolint:gocyclo
	...
	consensusStateProposalResult := makeNRequests(consensusStateProposalCount, consensusStateProposalDelay, func(_ int) bool {
		...
		responseCh := sms[nodeIndex].ConsensusStateProposal(context.Background(), bf.GetAccountOutput(blocks[blockIndex].L1Commitment()))
		<-responseCh
		return true
	})
	...
	consensusDecidedStateResult := makeNRequests(consensusDecidedStateCount, consensusDecidedStateDelay, func(_ int) bool {
		...
		responseCh := sms[nodeIndex].ConsensusDecidedState(context.Background(), bf.GetAccountOutput(blocks[blockIndex].L1Commitment()))
		state := <-responseCh
	...
	mempoolStateRequestResult := makeNRequests(mempoolStateRequestCount, mempoolStateRequestDelay, func(_ int) bool {
		...
		oldStateOutput := bf.GetAccountOutput(blocks[oldBlockIndex].L1Commitment())
		newStateOutput := bf.GetAccountOutput(blocks[newBlockIndex].L1Commitment())
